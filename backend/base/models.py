...
class Product(models.Model):
    ...
    image = models.FileField(
        validators=[FileExtensionValidator(['pdf', 'doc', 'svg', 'png', 'jpg', 'webp'])])
    featured = models.BooleanField(default=False)
    description = models.TextField(null=True, blank=True)
    price = models.DecimalField(max_digits=7, decimal_places=2)
    # rating = models.DecimalField(max_digits=7, decimal_places=2, blank=True)
    countInStockRetail = models.PositiveIntegerField(
        null=True, blank=True, default=0)
    countInStockWholesale = models.PositiveIntegerField(
        null=True, blank=True, default=0)
    ...
    def __str__(self) -> str:
        return f"{self.title} - {self.flavour}"
